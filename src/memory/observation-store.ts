...
function rowToObservation(row: ObservationRow): Observation {
  return {
    ...
    domain: row.domain as ObservationDomain | null,
    tier: (row.tier as MemoryTier) ?? 'project',
    content: row.content,
...
export class ObservationStore {
  ...
  add(input: NewObservation): Observation {
    ...
    const result = db.prepare(`
      INSERT INTO observations (session_id, worker_id, type, domain, tier, content, metadata, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(input.sessionId, input.workerId, input.type, domain, tier, input.content, metadata, createdAt);
    ...
    return {
      ...
      type: input.type,
      domain: domain as ObservationDomain | null,
  ...
  getByDomain(domain: ObservationDomain, limit: number = 100): Observation[] {
    ...
    const rows = db.prepare(`
      ...
      ORDER BY created_at DESC
      LIMIT ?
    `).all(domain, limit) as ObservationRow[];
  ...
  getByTier(tier: MemoryTier, limit: number = 100): Observation[] {
    ...
    const rows = db.prepare(`
      ...
      ORDER BY created_at DESC
      LIMIT ?
    `).all(tier, limit) as ObservationRow[];

    return rows.map(rowToObservation);
  }

  /**
   * Get patterns for a specific domain (learnings, conventions)
   */
  getDomainPatterns(domain: ObservationDomain, limit: number = 50): Observation[] {
    const db = this.database.getDb();
    const rows = db.prepare(`
      SELECT * FROM observations
      WHERE domain = ? AND type IN ('pattern', 'decision')
      ORDER BY created_at DESC
      LIMIT ?
    `).all(domain, limit) as ObservationRow[];
   ...
   * Get unique domains
   */
  getUniqueDomains(): ObservationDomain[] {
    const db = this.database.getDb();
    const rows = db.prepare(`
      SELECT DISTINCT domain FROM observations
      WHERE domain IS NOT NULL
      ORDER BY domain
    `).all() as { domain: string }[];

    return rows.map(r => r.domain as ObservationDomain);
  }
