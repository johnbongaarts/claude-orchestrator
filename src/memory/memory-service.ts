...
export class MemoryService {
  ...
  // ============================================
  // Pattern & Learning Methods (Three-Tier)
  // ============================================

  /**
   * Capture a reusable pattern or convention learned
   * This is the primary method for storing learnings
   */
  capturePattern(
    workerId: string,
    pattern: string,
    ...
    return this.captureObservation({
      ...
      tier,
      content: pattern,
      metadata: { domain, tier, ...metadata },
    });
  }

  /**
   * Capture an architectural or design decision
   */
  captureDecision(
    workerId: string,
    decision: string,
    ...
    return this.captureObservation({
      ...
      metadata: { domain, rationale, ...metadata },
    });
  }

  /**
   * Capture a session handoff summary for continuity
   */
  captureHandoff(
    workerId: string,
    summary: string,
    ...
    const content = pendingItems?.length
    ...
    return this.captureObservation({
      ...
      tier: 'project',
      content,
      metadata: { pendingItems, ...metadata },
    });
  }
   ...
   */
  getDomainPatterns(domain: ObservationDomain, limit: number = 50): Observation[] {
    this.ensureInitialized();
    return this.observations.getDomainPatterns(domain, limit);
  }

  /**
   * Get observations by domain
   */
  getByDomain(domain: ObservationDomain, limit: number = 100): Observation[] {
    this.ensureInitialized();
    return this.observations.getByDomain(domain, limit);
  }

  /**
   * Get observations by tier
   */
  getByTier(tier: MemoryTier, limit: number = 100): Observation[] {
    this.ensureInitialized();
    return this.observations.getByTier(tier, limit);
  }

  /**
   * Get unique domains that have observations
   */
  getDomains(): ObservationDomain[] {
    this.ensureInitialized();
    return this.observations.getUniqueDomains();
  }
